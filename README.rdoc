...
1. 'nokogiri' => HTML/XML Paser. (html 파서)
2. 'open-uri' => Get request to web page. (html 코드를 가져오도록 요청을 돕는 잼)
3. 'json'	  => Convert String to Json. (json 형태로 변환하기 위한 잼)
4. 'net/http' => Get Request to local page when background process. (background 작업 시 해당 서버에 get 요청을 보내기 위한 잼.)
5. 'whenever' => Crontab maker. (Crontab을 편하게 작성하도록 도와주는 잼.)
6. 'schema_to_scaffold' It helps making commandline that change existing schema to rails scaffold. (기존의 Mysql Schema를 Rails 프로젝트로 Dump하는 것을 도와주는 잼.)
